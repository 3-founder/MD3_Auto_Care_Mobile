...
class SignatureUserController {
  ...
  Future<void> addSignatureUser(
      String namaLengkap, List<int> tandaTangan) async {
    try {
      File imageFile = await _convertBytesToFile(tandaTangan);
  ...
  Future<File> _convertBytesToFile(List<int> tandaTangan) async {
    ...
    await file.writeAsBytes(tandaTangan);
